...
\fancypagestyle{BO}{%
  \fancyhf{}%
  \fancyhead[C]{\VAR{data.getVersion()}}
  \fancyhead[R]{\thepage}
  \fancyfoot[L]{©~Ministère de l'Enseignement supérieur, de la Recherche et de l'Innovation\\\ul{http://www.enseignementsup-recherche.gouv.fr}}
  %% set allletters=utils['parcoursLettres'](data.getParcoursObjects())
  \fancyfoot[R]{2021\\\VAR{data.getId()} (parcours \VAR{allletters|elegantjoin})}
...
Ce document présente le programme national du B.U.T \VAR{data.getId()}
...
\includepdf[nup=1x2,scale=.85,pages=-,pagecommand={\thispagestyle{BO}}]{assets/\VAR{data.getShortId()}_\VAR{p.getId()}.pdf}
%% endfor
...
\includepdf[scale=1,pages=-,pagecommand={\thispagestyle{BO}}]{assets/\VAR{data.getShortId()}_ANNEXEA_\VAR{p.getId()}.pdf}
%% endfor
...
par la spécialité « \VAR{data.getId()} » et les parcours associés. Enfin, le PPP prépare
...
\textbf{Semestre \VAR{sem}, \VAR{lname}}\\
%% for l in ll
\makebox[\linewidth]{\quad \hyperref[FICHE-\VAR{l.getId()}]{\VAR{l.getId()}~\VAR{l.getLongtxt()}}\dotfill\pageref{FICHE-\VAR{l.getId()}}}\par
%% endfor
%% endfor
%% endfor
