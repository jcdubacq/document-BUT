...
class LaTeXPrinter(Printer):
    def _specific(self):
        ...
        self.latexre=re.compile('|'.join(re.escape(str(key)) for key in sorted(self.latexconv.keys(), key = lambda item: - len(item))))
    ...
    def LaTeXEscape(self,a):
        ...
        if isinstance(a,list):
            b=[]
            for x in a:
                b.append(self.latexre.sub(lambda match: self.latexconv[match.group()], x))
            return b
        else:
